...
- (RACSignal *)requestNetworkData:(FMHttpRequest *)req{
    ...
    RACSignal *signal = [RACSignal createSignal:^(id<RACSubscriber> subscriber) {
        @strongify(self);
        /// 获取request
        NSString *url = [[NetWorkModel shared].BASE_URL stringByAppendingString:req.path];//KKK
        
        
        NSLog(@"请求URL:%@",url);//
        NSLog(@"parameters:%@",req.parameters);//
        NSError *serializationError = nil;
        NSLog(@"Mytoken = 【%@】",[NetWorkModel shared].TOKEN_VALUE_URL);
        // 小马特殊
        if ([NetWorkModel shared].TOKEN_VALUE_URL.length >0) {
            [self.manager.requestSerializer setValue:[NetWorkModel shared].TOKEN_VALUE_URL forHTTPHeaderField:[NetWorkModel shared].TOKEN_KEY_URL];
        }
        if ([req.method isEqualToString:HTTTP_METHOD_POST]) {
            
            [self.manager.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
        }
        if ([req.method isEqualToString:HTTTP_METHOD_GET]) {
            [[NSUserDefaults standardUserDefaults] setValue:@"GET" forKey:@"HTTTP_METHOD"];
        }
        else if ([req.method isEqualToString:HTTTP_METHOD_DELETE]) {
            [[NSUserDefaults standardUserDefaults] setValue:@"DELETE" forKey:@"HTTTP_METHOD"];
        }
        else
        {
            [[NSUserDefaults standardUserDefaults] setValue:@"Default" forKey:@"HTTTP_METHOD"];
        }
        [self.manager.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Accept"];
        
        NSString *jsonStr=[[NSString alloc]initWithData:[NSJSONSerialization dataWithJSONObject:req.parameters options:NSJSONWritingPrettyPrinted error:nil] encoding:NSUTF8StringEncoding];
        NSMutableURLRequest *request = [self.manager.requestSerializer requestWithMethod:req.method URLString:[[NSURL URLWithString:req.path relativeToURL:[NSURL URLWithString:url] ] absoluteString] parameters:jsonStr error:&serializationError];
...
//    [[LoginModel getUsingLKDBHelper] deleteToDB:[PublickDataManager sharedPublicDataManage].loginModel];
//    [[JCKTool shared] cleanCacheAndCookie];
//    [PublickDataManager sharedPublicDataManage].loginModel.token = @"";
    [MBProgressHUD wj_showError:text];
}
...
//    [[JCKTool shared] cleanCacheAndCookie];
//    [PublickDataManager sharedPublicDataManage].loginModel.token = @"";
    [NetWorkModel shared].TOKEN_VALUE_URL = @"";
}
